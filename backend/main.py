...
import json
import inspect
# import serial # No longer needed directly here for Rubik's
import asyncio
...
from utils.esp32_client import esp32_client # Global ESP32 client
...
TARGET_SHOOTER_DIR = os.path.join(BASE_DIR, "games") 
MEMORY_DIR = os.path.join(BASE_DIR, "games")
RUBIKS_DIR = os.path.join(BASE_DIR, "games") # For rubiks_cube_game if it has local imports

if TICTACTOE_DIR not in sys.path: sys.path.insert(0, TICTACTOE_DIR)
if TARGET_SHOOTER_DIR not in sys.path: sys.path.insert(0, TARGET_SHOOTER_DIR)
if MEMORY_DIR not in sys.path: sys.path.insert(0, MEMORY_DIR)
if RUBIKS_DIR not in sys.path: sys.path.insert(0, RUBIKS_DIR) # Ensure games dir is in path for Rubik's utils if any

GAME_MODULES = {
    "shell-game": "games.shellGame",
    "tic-tac-toe": "games.tic-tac-toe.tictactoe",
    "rubiks": "games.rubiks_cube_game", # Module path for Rubik's
    "target-shooter": "games.target_shooter_game",
    "color": "games.memory_matching_backend",
    "yolo": "games.memory_matching_backend",
}
    ...
    try:
        if game_id in ["color", "yolo"]:
            ...
            if memory_game_instances[game_id].running:
                await websocket.send_json({"type": "error", "payload": f"{game_id.capitalize()} game is busy. Please try again later."})
            ...
            # ... (existing code for other games, ensure esp32_client is passed if needed) ...
            ...
            if game_id == "shell-game":
                ...
                # This part requires GameSession to potentially accept esp32_client
                game_session_args = []
                game_session_kwargs = {}
        ...
        while True:
            ...
            try:
                if game_id == "rubiks" and "text" in data and data["text"] is not None:
                    ...
                    if game_id == "rubiks":
                        # process_frame is now async, use maybe_await or direct await
                    ...
                    if game_id != "rubiks":
                        # ... (existing command handling for other games) ...
                        ...
                        try: parsed_command = json.loads(command_content)
                            ...
                            else: parsed_command = command_content
...
if __name__ == "__main__":
    ...
    # This needs to be run in an asyncio event loop if connect is called here.
    # For uvicorn, it's better to let it connect when first needed or in an app startup event.
    # async def main_async():
    #    await esp32_client.connect() # Attempt initial connection
    # asyncio.run(main_async())
    
    # Uvicorn will run its own event loop. ESP32 client will connect on first use.
