...
export default function RubiksSolverPage() {
  ...
  const sendNextFrameRef = useRef(false); // Start with false to delay first frame
  ...
  const sendCommand = useCallback(
    (command) => {
      console.log(`[sendCommand] Attempting to send command: ${command}`); // **DEBUG**
      if (isActionLoading) {
        console.warn(
          `[sendCommand] Action already in progress, skipping command: ${command}`
        ); // **DEBUG**
        return;
      }
      ...
      if (!ws || ws.readyState !== WebSocket.OPEN) {
        console.error(
          `[sendCommand] WebSocket not open (State: ${
            ws?.readyState ?? "null"
          }). Cannot send command: ${command}`
        ); // **DEBUG**
        setStatus((prev) => ({
          ...prev,
          mode: "error",
          status_message: "Connection Error",
          error_message: "WebSocket connection is not open.",
        }));
        return;
      }
      setIsActionLoading(true);
      console.log(`[sendCommand] Sending command: ${command}`); // **DEBUG**
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      console.log("[connectWebSocket] Attempting connection to:", WS_URL); // **DEBUG**
      if (stopped) {
      ...
      try {
        ...
        ws.onopen = () => {
          ...
          // Delay sending the first frame to allow backend to stabilize
          setTimeout(() => {
        ...
        ws.onmessage = (event) => {
          console.log("[WS] Received message:", event.data); // **DEBUG**
          ...
          try {
            ...
            if (data.status === "error") {
              setStatus((prev) => ({
                ...prev,
                mode: "error",
                status_message: data.message || "Backend error",
                error_message: data.message || "Unknown error",
                serial_connected:
                  data.state?.serial_connected ?? prev.serial_connected,
              }));
              return;
            }
            ...
            setStatus((prev) => ({
              ...prev,
              mode: data.state?.mode ?? prev.mode,
              status_message: data.message ?? prev.status_message,
              error_message: null,
              solution: data.state?.solution ?? prev.solution,
              serial_connected:
                data.state?.serial_connected ?? prev.serial_connected,
              calibration_step:
                data.state?.calibration_step ?? prev.calibration_step,
              current_color: data.state?.current_color ?? prev.current_color,
              scan_index: data.state?.scan_index ?? prev.scan_index,
              solve_move_index:
                data.state?.solve_move_index ?? prev.solve_move_index,
              total_solve_moves:
                data.state?.total_solve_moves ?? prev.total_solve_moves,
            }));
        ...
        console.error("[connectWebSocket] Failed to create WebSocket:", error); // **DEBUG**
        setWsConnectionStatus("Error");
        setStatus((prev) => ({
          ...prev,
          mode: "error",
          status_message: "Connection Error",
          error_message: "Failed to initialize WebSocket connection.",
        }));
        if (!stopped) {
          console.log("[WS] Scheduling reconnect in 5s..."); // **DEBUG**
          reconnectTimeout = setTimeout(connectWebSocket, 5000);
    ...
    initCamera();
    connectWebSocket();
    // Delay frame sending to allow WebSocket to stabilize
    setTimeout(() => {
      frameRequestId = requestAnimationFrame(sendFrame);
    }, 1000); // 1-second delay
  ...
  const handleCalibrate = () => {
    console.log("[Click] Calibrate"); // **DEBUG**
    sendCommand("calibrate_color");
  };

  const handleStartSolve = () => {
    console.log("[Click] Start Solve"); // **DEBUG**
    sendCommand("start_scanning");
  };

  const handleStopAndReset = () => {
    console.log("[Click] Stop & Reset"); // **DEBUG**
    sendCommand("stop");
  };

  const handleStartScramble = () => {
    console.log("[Click] Start Scramble"); // **DEBUG**
    sendCommand("scramble_cube");
  };
      ...
      <div className="w-full max-w-5xl bg-white rounded-xl shadow-lg p-6 mt-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 border-b border-gray-200 pb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => {
                console.log("[Click] Open Settings"); // **DEBUG**
          ...
          <p className="text-sm font-medium text-gray-700">
            ...
            {status.mode === "calibrating" &&
              status.calibration_step != null && (
                <span className="ml-2 text-xs font-normal text-gray-500">
                  (Color {status.calibration_step + 1} / {COLOR_NAMES.length}:{" "}
            ...
            <div className="relative w-[320px] h-[240px] rounded-lg overflow-hidden border-2 border-gray-300 bg-black flex items-center justify-center shadow-inner">
              {appliedCameraSettings.useIpCamera && (
                <img
                  ...
                  onError={(e) => {
                    console.error("[IP Cam] onError event triggered:", e); // **DEBUG**
                    setIsCameraLoading(false);
              ...
              {!appliedCameraSettings.useIpCamera && (
                <video
                  ...
                  onError={(e) => {
                    console.error("[Device Cam] onError event triggered:", e); // **DEBUG**
                    setIsCameraLoading(false);
                  }}
                />
              )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="border border-gray-200 p-4 rounded-lg bg-gray-50 shadow-sm">
            ...
            <div className="flex flex-col gap-2">
              ...
              >
                {status.mode === "calibrating"
                  ? `Calibrate '${status.current_color || "?"}'`
                  : "Start Calibration"}
              </button>
            </div>
          </div>
