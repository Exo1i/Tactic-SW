...
export default function RubiksSolverPage() {
    ...
    const [appliedCameraSettings, setAppliedCameraSettings] = useState({
        useIpCamera: false, ipCameraUrl: "http://192.168.1.9:8080/video", // **DEBUG** Default IP for testing
    });
    ...
    const initCamera = useCallback(async () => {
        console.log("%c[initCamera] Start", "color: blue; font-weight: bold;", appliedCameraSettings); // **DEBUG**
        ...
        if (!appliedCameraSettings.useIpCamera) {
            ...
            try {
                ...
                console.log("[initCamera] Requesting media device with constraints:", constraints); // **DEBUG**
                ...
                if (videoRef.current) {
                    ...
                    console.error("[initCamera] ERROR: videoRef is null when assigning stream."); // **DEBUG**
                    stream.getTracks().forEach(track => track.stop());
                    setIsCameraLoading(false); // Cannot proceed
                }
            } catch (error) {
                console.error("[initCamera] getUserMedia ERROR:", error.name, error.message, error); // **DEBUG** Enhanced logging
                let errorMsg = `Failed to access camera: ${error.name}.`;
                if (error.name === "NotAllowedError") errorMsg = "Camera permission denied. Please allow access."; else if (error.name === "NotFoundError") errorMsg = "No suitable camera found."; else if (error.name === "NotReadableError") errorMsg = "Camera is already in use or hardware error."; else if (error.name === "OverconstrainedError") errorMsg = `Camera does not support requested resolution/facingMode. ${error.message}`; else if (error.name === "AbortError") errorMsg = "Camera request aborted.";
                setStatus(prev => ({...prev, mode: "error", status_message: "Camera Error", error_message: errorMsg}));
                setIsCameraLoading(false);
            }
        } else {
            console.log("[initCamera] Using IP Camera:", appliedCameraSettings.ipCameraUrl); // **DEBUG**
    ...
    useEffect(() => {
        console.log("%c[useEffect] Component Mounted / appliedCameraSettings changed", "color: green; font-weight: bold;"); // **DEBUG**
        let stopped = false;
        ...
        const connectWebSocket = () => {
            ...
            ws.onclose = (event) => {
                console.warn(`%c[WS] Disconnected: Code=${event.code}, Reason='${event.reason}'`, "color: orange;"); // **DEBUG**
            ...
            ws.onmessage = (event) => {
                ...
                try {
                    ...
                    console.error("[WS] Failed to parse message:", e, "Raw data:", event.data); // **DEBUG** Log raw data on parse failure
        ...
        const sendFrame = () => {
            ...
            let skipReason = null;
            if (!sendNextFrameRef.current) skipReason = "sendNextFrameRef is false (waiting for backend response)"; else if (!ws || ws.readyState !== WebSocket.OPEN) skipReason = `WebSocket not ready (State: ${ws?.readyState ?? 'null'})`; else if (now - lastSentRef.current < minFrameInterval) skipReason = "Frame interval too short";
            ...
            if (appliedCameraSettings.useIpCamera) {
                ...
                if (video && video.readyState >= video.HAVE_ENOUGH_DATA) { // Use readyState directly
                    sourceEl = video;
            ...
            if (!sourceEl) {
                console.warn("[sendFrame] No valid source element found (this shouldn't happen if sourceReady is true).");
                    ...
                    if (blob && ws.readyState === WebSocket.OPEN) {
                        ...
                        blob.arrayBuffer().then((buffer) => {
                            ...
                            if (ws.readyState === WebSocket.OPEN) {
                                ...
                                console.log(`%c[sendFrame #${frameSentCounter}] Sending buffer (Size: ${buffer.byteLength}) via WebSocket...`, "color: purple;"); // DEBUG Highlight send
                                ws.send(buffer);
                            } else {
                                console.warn("[sendFrame] WebSocket closed before buffer could be sent."); // DEBUG
                        ...
                        console.warn("[sendFrame] Blob creation failed or WebSocket closed in toBlob callback."); // DEBUG
                        sendNextFrameRef.current = true; // Allow next attempt
                        setIsBackendProcessing(false);
                        if (!blob) console.warn("[sendFrame] Blob is null in toBlob callback.");
                        if (ws.readyState !== WebSocket.OPEN) console.warn(`[sendFrame] WebSocket not open in toBlob callback (State: ${ws.readyState}).`);
                    }
                }, "image/jpeg", 0.7);
        ...
        return () => {
            stopped = true;
            console.log("%c[useEffect Cleanup] Running cleanup...", "color: red; font-weight: bold;"); // **DEBUG**
            if (frameRequestId) {
                console.log("[useEffect Cleanup] Cancelling animation frame request:", frameRequestId); // **DEBUG**
                cancelAnimationFrame(frameRequestId);
                frameRequestId = null;
            }
            if (reconnectTimeout) {
            ...
            console.log("%c[useEffect Cleanup] Cleanup complete.", "color: red; font-weight: bold;"); // **DEBUG**
        };
    ...
    const callApi = useCallback(async (endpoint, method = 'POST', body = null) => {
        ...
        if (isActionLoading) {
            console.warn(`[callApi] Action already in progress, skipping call to ${endpoint}`); // **DEBUG**
            return;
        }
        ...
        loadingTimeoutRef.current = setTimeout(() => {
            console.warn(`[callApi] SAFETY TIMEOUT triggered for ${endpoint} - resetting loading state`); // **DEBUG**
        ...
        try {
            ...
            console.log(`[callApi] Response status for ${endpoint}: ${response.status}`); // **DEBUG**
            const data = await response.json(); // Attempt to parse JSON regardless of status

            if (!response.ok) {
                console.error(`[callApi] API Error ${response.status} for ${endpoint}:`, data); // **DEBUG** Log error data
            ...
            if (['/start_scramble', '/start_solve', '/start_calibration'].includes(endpoint)) {
    ...
    const applySettings = () => {
        console.log("[Settings] Applying new camera settings:", modalCameraSettings); // **DEBUG**
    ...
    return (<div className="flex flex-col items-center gap-4 p-4 min-h-screen bg-gray-100 font-sans">
        {/* Settings Modal */}
        {showSettings && (<div
                ...
                <div className="flex items-center mb-3">
                    <input
                        ...
                        onChange={(e) => setModalCameraSettings({
                            ...modalCameraSettings, useIpCamera: e.target.checked
                        })}
                        className="mr-2 h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    />
                    <label htmlFor="useIpCameraModal" className="text-sm font-medium text-gray-700">Use IP
                                                                                                    Camera</label>
                </div>
                {modalCameraSettings.useIpCamera && (<div className="mb-4">
                    <label htmlFor="ipCameraUrlModal"
                           className="block mb-1 text-sm font-medium text-gray-700">IP Camera URL:</label>
                    <input
                        type="text"
                        id="ipCameraUrlModal"
                        value={modalCameraSettings.ipCameraUrl}
                        onChange={(e) => setModalCameraSettings({
                            ...modalCameraSettings, ipCameraUrl: e.target.value
                        })}
                    ...
                    />
                    <small className="text-xs text-gray-500 mt-1 block">E.g.,
        ...
        <div className="w-full max-w-5xl bg-white rounded-xl shadow-lg p-6 mt-4">
            ...
            <div
                ...
                <div className="flex items-center gap-3">
                    ...
                    <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${wsConnectionStatus === "Connected" ? "bg-green-100 text-green-800" : wsConnectionStatus === "Disconnected" ? "bg-red-100 text-red-800" : wsConnectionStatus === "Connecting..." ? "bg-yellow-100 text-yellow-800 animate-pulse" : "bg-gray-100 text-gray-800"}`}> WS: {wsConnectionStatus} </span>
                    <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${status.serial_connected ? "bg-blue-100 text-blue-800" : "bg-red-100 text-red-800"}`}> {status.serial_connected ? "Serial OK" : "Serial disconnected"} </span>
                    <button
                        onClick={() => {
            ...
            <div className="mb-5 p-3 bg-gray-50 rounded-lg border border-gray-200 shadow-sm">
                <p className="text-sm font-medium text-gray-700">
                    ...
                    {(status.mode === 'solving' || status.mode === 'scrambling') && status.total_solve_moves > 0 && (
                        <span
                            className="ml-2 text-xs font-normal text-gray-500"> ({status.solve_move_index || 0} / {status.total_solve_moves}) </span>)}
                    {status.mode === 'scanning' && status.scan_index != null && (
                        <span
                            className="ml-2 text-xs font-normal text-gray-500"> (Scan {status.scan_index + 1} / 12) </span>)}
                </p>
                <p className="text-sm mt-1 text-gray-600 min-h-[1.25rem]"> {status.status_message || " "} </p>
                {status.mode === 'error' && status.error_message && (
                    <p className="text-sm mt-1 text-red-700 font-medium bg-red-50 p-2 rounded border border-red-200"> Error: {status.error_message} </p>)}
                {status.solution && (<div
                    className="mt-2 text-sm text-blue-800 font-mono bg-blue-50 p-2 rounded border border-blue-200 max-h-24 overflow-y-auto">
                    <span className="font-semibold">Solution Found:</span> <p
                    className="whitespace-pre-wrap break-words text-xs leading-relaxed">{status.solution}</p>
                </div>)}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="flex flex-col items-center">
                    ...
                    <div
                        ...
                        {appliedCameraSettings.useIpCamera && (<img
                            ref={ipCamImgRef}
                            id="ip-camera-img-display"
                            // src is set in initCamera
                            alt="IP Camera Feed"
                            width={320} height={240} className="object-contain" crossOrigin="anonymous"
                            onLoad={() => {
                            ...
                            onError={(e) => {
                                ...
                                setIsCameraLoading(false);
                                if (status.mode !== 'error') setStatus(prev => ({
                                    ...prev,
                        ...
                        />)}
                        {!appliedCameraSettings.useIpCamera && (<video
                            ref={videoRef}
                            autoPlay playsInline muted
                            width={320} height={240} className="object-contain" style={{background: "#222"}}
                            onCanPlay={() => {
                        ...
                        {!isCameraLoading && status.mode === "error" && status.error_message?.toLowerCase().includes("camera") && (
                            <div
                                ...
                                <div className="p-3 text-center rounded bg-red-800 bg-opacity-90 shadow"><span
                                    className="font-bold block text-base">Camera Error</span> <span
                                    className="text-xs mt-1 block">{status.error_message}</span></div>
                            </div>)}
                ...
                <div className="flex flex-col items-center">
                    ...
                    <div className="mb-1 text-center text-sm font-medium text-gray-600">Processed View (Backend)
                    </div>
                    <div
                        className="relative w-[320px] h-[240px] rounded-lg overflow-hidden border-2 border-gray-300 bg-black flex items-center justify-center shadow-inner">
                        ...
                        {isBackendProcessing && (<div
                            className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-60 text-white text-xs p-1 animate-pulse"> Backend
                                                                                                                                                       Processing... </div>)}
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="border border-gray-200 p-4 rounded-lg bg-gray-50 shadow-sm">
                    ...
                    <div className="flex flex-col gap-2">
                        <button onClick={handleStartCalibration} disabled={actionDisabled}
                                className="btn bg-indigo-500 hover:bg-indigo-600 disabled:bg-gray-400"> Start
                                                                                                        Calibration
                        </button>
                        {status.mode === 'calibrating' && (<>
                            <button onClick={handleCaptureColor}
                                    disabled={isActionLoading || (status.calibration_step == null || status.calibration_step >= COLOR_NAMES.length)}
                                    className="btn bg-teal-500 hover:bg-teal-600 disabled:bg-gray-400"> Capture
                                                                                                        '{status.current_color || '?'}'
                            </button>
                            <button onClick={handleSaveCalibration}
                                    disabled={isActionLoading || (status.calibration_step == null || status.calibration_step < COLOR_NAMES.length)}
                                    className="btn bg-green-500 hover:bg-green-600 disabled:bg-gray-400"> Save
                                                                                                          Calibration
                            </button>
                        </>)}
                        <button onClick={handleResetCalibration} disabled={isActionLoading}
                                className="btn bg-yellow-500 hover:bg-yellow-600 disabled:bg-gray-400 mt-2"> Reset
                                                                                                             Calibration
                        </button>
                    </div>
                </div>
                ...
                <div className="border border-gray-200 p-4 rounded-lg bg-gray-50 shadow-sm">
                    ...
                    <div className="flex flex-col gap-2">
                        <button onClick={handleStartSolve} disabled={actionDisabled || !status.serial_connected}
                                title={!status.serial_connected ? "Serial disconnected" : ""}
                                className={`btn bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 ${!status.serial_connected ? 'cursor-not-allowed' : ''}`}> Solve
                                                                                                                                                              Cube
                        </button>
                        <button onClick={handleStopAndReset}
                                disabled={status.mode === 'idle' || status.mode === 'connecting' || isActionLoading}
                                className="btn bg-orange-500 hover:bg-orange-600 disabled:bg-gray-400 mt-2"> Stop &
                                                                                                             Reset
                        </button>
                    </div>
                </div>
                ...
                <div className="border border-gray-200 p-4 rounded-lg bg-gray-50 shadow-sm">
                    ...
                    <div className="flex flex-col gap-2">
                        <button onClick={handleStartScramble} disabled={actionDisabled || !status.serial_connected}
                                title={!status.serial_connected ? "Serial disconnected" : ""}
                                className={`btn bg-red-500 hover:bg-red-600 disabled:bg-gray-400 ${!status.serial_connected ? 'cursor-not-allowed' : ''}`}> Scramble
                                                                                                                                                            Cube
                        </button>
                    </div>
                </div>
            ...
            {isActionLoading && (<div className="mt-4 text-center text-sm text-gray-600 animate-pulse font-medium">
                Waiting for backend response... {/* **DEBUG** This should appear when buttons are clicked */}
            </div>)}
        </div>
