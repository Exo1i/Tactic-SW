...
export default function ShellGamePage() {
  ...
  async function handleStartCalibration() {
    ...
    setError(null);
    try {
      const res = await fetch("http://localhost:8000/start_calibration", {
        method: "POST",
      });
      ...
      <div className="w-full max-w-4xl bg-white rounded-xl shadow-lg p-6 mt-4">
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg border">
          ...
          <pre className="text-sm overflow-x-auto">
            {gameState ? JSON.stringify(gameState, null, 2) : "No data"}
          </pre>
        </div>
        <button onClick={handleStartCalibration} disabled={loading}>
